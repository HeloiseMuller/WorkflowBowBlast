...
cat $dir/Coverage/${sample}_trimmed_vs_${species}_coverage_positions | awk -v a=$species -v b=$sample '{sum+=$3} END {print "Average coverage of " b " on " a " = ",sum/NR}' 
...
then
	...
	cat $dir/Coverage/${sample}_trimmed_vs_${species2}_coverage_positions | awk -v a=$species2 -v b=$sample '{sum+=$3} END {print "Average coverage of " b " on " a " = ",sum/NR}' 
